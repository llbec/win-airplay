...
#define TEXT_INPUT_NAME     obs_module_text("IOSDevice")
...
static DWORD CALLBACK AirPlayThread(LPVOID ptr);
...
struct AirPlayInput {
	...
	inline AirPlayInput(obs_source_t *source_, obs_data_t *settings)
		: source(source_), device(InitGraph::False)
	{
		...
		thread =
			CreateThread(nullptr, 0, AirPlayThread, this, 0, nullptr);
		if (!thread)
...
static DWORD CALLBACK AirPlayThread(LPVOID ptr)
{
	AirPlayInput *airplayInput = (AirPlayInput *)ptr;

	os_set_thread_name("win-airplay: AirPlayThread");

	CoInitialize(nullptr);
	airplayInput->AirPlayLoop();
	CoUninitialize();
	return 0;
}
...
void AirPlayInput::OnEncodedVideoData(enum AVCodecID id, unsigned char *data,
				    size_t size, long long ts)
{
...
void AirPlayInput::OnVideoData(const VideoConfig &config, unsigned char *data,
...
void AirPlayInput::OnEncodedAudioData(enum AVCodecID id, unsigned char *data,
				    size_t size, long long ts)
{
...
void AirPlayInput::OnAudioData(const AudioConfig &config, unsigned char *data,
			     size_t size, long long startTime,
			     long long endTime)
{
	size_t block_size;
...
inline void AirPlayInput::SetupBuffering(obs_data_t *settings)
{
...
bool AirPlayInput::UpdateVideoConfig(obs_data_t *settings)
{
	...
	videoConfig.callback = std::bind(&AirPlayInput::OnVideoData, this,
	...
	blog(LOG_INFO, "---------------------------------");
	blog(LOG_INFO,
	     "[AirPlay Device: '%s'] settings updated: \n"
	     "\tresolution: %dx%d\n"
	     "\tflip: %d\n"
...
bool AirPlayInput::UpdateAudioConfig(obs_data_t *settings)
{
	bool useCustomAudio = obs_data_get_bool(settings, USE_CUSTOM_AUDIO);

	if (!useCustomAudio && !deviceHasAudio) {
		return true;
	}
	...
	audioConfig.callback = std::bind(&AirPlayInput::OnAudioData, this,
					 placeholders::_1, placeholders::_2,
					 placeholders::_3, placeholders::_4,
					 placeholders::_5);
...
void AirPlayInput::SetActive(bool active_)
{
...
AirPlayInput::GetColorSpace(obs_data_t *settings) const
{
...
AirPlayInput::GetColorRange(obs_data_t *settings) const
{
...
inline bool AirPlayInput::Activate(obs_data_t *settings)
{
...
inline void AirPlayInput::Deactivate()
{
	device.ResetGraph();
	obs_source_output_video2(source, nullptr);
}
...
static const char *GetAirPlayInputName(void *)
{
...
static void proc_activate(void *data, calldata_t *cd)
{
	...
	AirPlayInput *input = reinterpret_cast<AirPlayInput *>(data);
	input->SetActive(activate);
}

static void *CreateAirPlayInput(obs_data_t *settings, obs_source_t *source)
{
	AirPlayInput *airplay = nullptr;

	try {
		airplay = new AirPlayInput(source, settings);
		proc_handler_t *ph = obs_source_get_proc_handler(source);
		proc_handler_add(ph, "void activate(bool active)",
				 proc_activate, airplay);
...
static void DestroyAirPlayInput(void *data)
{
	delete reinterpret_cast<AirPlayInput *>(data);
}

static void UpdateAirPlayInput(void *data, obs_data_t *settings)
{
	AirPlayInput *input = reinterpret_cast<AirPlayInput *>(data);
	if (input->active)
...
static void GetAirPlayDefaults(obs_data_t *settings)
{
				  ...
				  obs_data_t *settings)
{
	...
	VideoDevice device = data->device;
...
static obs_properties_t *GetAirPlayProperties(void *obj)
{
	AirPlayInput *input = reinterpret_cast<AirPlayInput *>(obj);
	...
	obs_property_t *p = obs_properties_add_list(ppts, "IOS_DEVICE",
						    TEXT_DEVICE,
						    OBS_COMBO_TYPE_LIST,
...
void AirPlayModuleLogCallback(LogType type, const wchar_t *msg, void *param)
{
	...
	blog(obs_type, "AirPlay: %s", dmsg->array);

	UNUSED_PARAMETER(param);
}

static void HideAirPlayInput(void *data)
{
	AirPlayInput *input = reinterpret_cast<AirPlayInput *>(data);
...
static void ShowAirPlayInput(void *data)
{
	AirPlayInput *input = reinterpret_cast<AirPlayInput *>(data);
...
void RegisterAirPlaySource()
{
	SetLogCallback(AirPlayModuleLogCallback, nullptr);
	...
	info.show = ShowAirPlayInput;
	info.hide = HideAirPlayInput;
	info.get_name = GetAirPlayInputName;
	info.create = CreateAirPlayInput;
	info.destroy = DestroyAirPlayInput;
	info.update = UpdateAirPlayInput;
	//info.get_defaults = GetAirPlayDefaults;
	//info.get_properties = GetAirPlayProperties;
