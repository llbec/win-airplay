...
struct airplay_callbacks_s {
	...
	void(*AirPlayAudio_Process)(void *cls, const void *buffer, int buflen, uint64_t timestamp, uint32_t seqnum);
	...
	void(*AirPlayMirroring_Play)(void *cls, int width, int height, const void *buffer, int buflen, int payloadtype, uint64_t timestamp);
	void(*AirPlayMirroring_Process)(void *cls, const void *buffer, int buflen, int payloadtype, uint64_t timestamp);
...
int AIR_API startMediaServer(char *friendname, int width, int height, airplay_callbacks_t *cb);
